...
def ping_ip(ip):
    try:
        output = subprocess.check_output(['ping', PING_PARAM, '1', TIMEOUT_PARAM, '1', str(ip)], stderr=subprocess.STDOUT, universal_newlines=True)
...
def scan_network(network):
    ...
    broadcast_ip = str(net.broadcast_address) # Get the broadcast IP address of the network
    ...
    for ip in arp_table:
        try:
            if ipaddress.ip_address(ip) in net and ip != broadcast_ip and ip not in ip_list:
                ip_list.append(ip)
...
def main():
    ...
    if ip_list:
        ...
        logging.info("No active IPs found.")
